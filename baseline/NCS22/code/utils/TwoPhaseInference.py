...
# 从环境变量中获取 n_jobs
import os
n_jobs = int(os.getenv('NCS22_NJOBS', -1))  # 默认值为1，如果环境变量未设置
...
def library_matching(df1,df2):
    ...
    for ii in df1.keys():
        if ii != 'constant':
            new_data = pd.concat([new_data,df2[ii]],axis=1)
    return new_data

def terms_sort_Lars(X_lib,Y_goal,intercept):
    reg = LassoLarsCV(cv=5, fit_intercept=intercept,  n_jobs=n_jobs, max_iter=1000, normalize=False).fit(X_lib,Y_goal)
...
def terms_sort_Lasso(X_lib,Y_goal,intercept):
    reg = LassoCV(cv=5, fit_intercept=intercept, n_jobs=n_jobs, max_iter=1000).fit(X_lib,Y_goal)
    coef = pd.Series(reg.coef_, index=X_lib.columns)
    if intercept == True:
        coef['constant'] = reg.intercept_
        num_params = len(coef)
...
def random_batch_generation(X,y,Number,batch_number):
    ...
    for i in range(0,batch_number):
        ...
        X_sample = pd.concat([X_sample,tmp1], axis=0)
        y_sample = pd.concat([y_sample,tmp2], axis=0)
    return X_sample, y_sample, sample_list

def PhaseOne(FuncMatrix, NumDiv, Nnodes, dim, Dim, keep, Lambda, plotstart, plotend, verbose=True):
    X_all = FuncMatrix*1
    y_all = NumDiv*1
    ...
    if Dim>=3:
        X = X_all.iloc[T_len*0:T_len*20,:]
        y = y_all.iloc[T_len*0:T_len*20,:]
    else:
        X = X_all
        y = y_all
    ...
    reg1 = LassoCV(cv=5, fit_intercept=True, n_jobs=n_jobs, max_iter=10000).fit(Xin, y1)
    if verbose:
        print('Score: %.3f' % reg1.score(Xin,y1))
    ...
    imp_ = imp_[~imp_.index.duplicated()]
    ...
    imp_coef1 = imp_coef1[~imp_coef1.index.duplicated()]
    ...
    imp_coef1_1 = imp_coef1_1[~imp_coef1_1.index.duplicated()]
    ...
    if verbose: print(aic_constant, aic_no_con, aic_1)
    con1 = aic_constant/aic_constant
    no_con = aic_no_con/aic_constant
    con2 = aic_1/aic_constant
    if verbose: print(con1,no_con,con2)
    if aic_constant<=0 and aic_no_con>=0:
        intercept = True
        if verbose: print("This equation may contain a constant term.")
        ...
        if con1-no_con>Lambda[dim,0] and abs(con1-con2)<=Lambda[dim,1]: 
            ...
            intercept = False
            if verbose: print("This equation may not contain a constant term.")
    elif aic_constant>0 and aic_no_con<0:
        intercept = False
        if verbose: print("This equation may not contain a constant term.")
    elif aic_constant>0 and aic_no_con>=0:
...
def PhaseTwo(Matrix, NumDiv, Nnodes, dim, keep, SampleTime, batchsize, Lambda, intercept,imp_cons,imp_no_cons, verbose=True):
    weight = 10
    Lambda = Lambda.values
    ...
    for ii in range(0,SampleTime):
        ...
        if intercept == True:
            ...
            if verbose: print(new_data.columns)
        ...
        if verbose: print("orinorm_aic:", orinorm_aic)
    ...
    if verbose: print(InferredResults)
...
def TwoPhaseInference(FuncMatrix, NumDiv, Nnodes, dim, Dim, keep, SampleTime, batchsize, Lambda, plotstart, plotend, verbose=True):
    PhaseOne_series,intercept,imp_cons,imp_no_cons = PhaseOne(FuncMatrix, NumDiv, Nnodes, dim, Dim, keep, Lambda, plotstart, plotend, verbose)
    keep = len(PhaseOne_series.columns)
    InferredResults, aic_final = PhaseTwo(FuncMatrix, NumDiv, Nnodes, dim, keep, SampleTime, batchsize, Lambda, intercept,imp_cons,imp_no_cons, verbose)
