...
def main(args):
    ...
    mask = np.full(data[args.target_var].shape, False)
    mask[:, data['c'] == args.class_idx] = True
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--name', type=str, default=f'SearchHetero_{time.strftime("%Y%m%d_%H%M%S")}')
    parser.add_argument('-d', '--device', type=str, default='auto')
    parser.add_argument('-s', '--seed', type=int, default=None)
    parser.add_argument('--data', type=str, default='./data/hetero/KUR_MM.json')
